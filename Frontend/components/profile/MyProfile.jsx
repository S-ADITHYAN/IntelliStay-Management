...
const MyProfile = () => {
  ...
  const handleSave = () => {
    ...
    axios.post(`http://localhost:3001/profile/update/${userData._id}`, profileData)
      .then(response => {
  ...
  const handleImageUpload = (event) => {
    ...
    axios.post(`http://localhost:3001/upload-photo/${userData._id}`, formData)
      .then(response => {
      ...
      >
        <Box display="flex" flexDirection="column" alignItems="center" mb={4} position="relative">
          <Avatar
            ...
            src={profileData.image ? `http://localhost:3001/profilepicture/${profileData.image}` : "/placeholder-avatar.png"}
            sx={{
              width: 150,
              height: 150,
              mb: 1,
