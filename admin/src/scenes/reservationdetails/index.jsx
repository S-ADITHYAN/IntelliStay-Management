...
const ReservationDetails = () => {
  ...
  useEffect(() => {
    const fetchBookingDetails = async () => {
      ...
      try {
        const response = await axios.get(`${import.meta.env.VITE_API}/admin/user-booking/${id}`);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          const response = await axios.post(`${import.meta.env.VITE_API}/admin/user-bookings/cancel/${reservation._id}`);
  ...
  const handleUpdateDocument = (guestId) => {
    ...
    fileInput.onchange = async (event) => {
      ...
      if (file) {
        ...
        try {
          const response = await axios.post(`${import.meta.env.VITE_API}/admin/user-guests-proofupdate/${guestId}`, formData, {
            headers: {
  ...
  return (
    <>
      ...
      <Box p={4}>
        <Typography variant="h4" gutterBottom style={{ fontWeight: "bold", textAlign: "center", position: "relative" }}>
          Booking Details
          <Box
    component="span"
    sx={{
