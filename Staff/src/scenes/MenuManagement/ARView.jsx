...
const ARView = ({ onClose, menuItems = [] }) => {
  ...
  // Modify the initScene useEffect to prevent unnecessary reinitializations
  useEffect(() => {
    ...
    initScene();
  }, [selectedARItem, isLoading]);

  // Modify camera initialization to prevent multiple attempts
  useEffect(() => {
    let isActive = true;

    const initCamera = async () => {
      if (!isActive || videoRef.current?.srcObject) return; // Skip if already initialized
  ...
  const addItemToAR = (menuItem) => {
    ...
    setAvailableItems(prev => prev.filter(item => item._id !== menuItem._id));
